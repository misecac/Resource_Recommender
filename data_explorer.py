...
def IsScheduleAvailable(data_day=[],data_week={},week_id='',cronlist=[],duration=0):
    ...
    for index in cronlist:
        ...
        if end > num_elements:
            ...
            result = result and IsAvailable(data_day,data_week,0,newend,week_id)
            result = result and IsAvailable(data_day,data_week,start,end,week_id)
        else:
            result = result and IsAvailable(data_day,data_week,start,end,week_id)
...
def generate_schedule_table(cron_data={},duration=0,map_day=['0']*num_elements,map_week={}):
    for cron in cron_data:        
        ...
        if not IsScheduleAvailable(cronlist=cron_data[cron],duration=duration,data_day=map_day,data_week=map_week, week_id=cron):
            print cron_data[cron]
            print 'unavailable'
            break
        
        for index in range(0,len(value)):            
            start,end = get_boundary_times(value[index],duration)
